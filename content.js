...
const SITE_CONTENT = {
  ...
  // ===== 교육 정보 =====
  education: {
    'academic': `<div class="flex flex-col md:flex-row gap-6 p-8 rounded-3xl bg-slate-50 border border-slate-100 hover:border-brand-100 transition duration-300 animate-[slideIn_0.4s_ease-out]"><div class="w-16 h-16 rounded-2xl bg-white shadow-sm flex items-center justify-center text-3xl border border-slate-100">🎓</div><div class="flex-1"><div class="flex flex-col md:flex-row md:items-center justify-between mb-2"><h4 class="text-xl font-bold text-slate-900 break-keep"></h4><span class="text-xs font-bold text-primary-600 bg-brand-50 px-3 py-1 rounded-full w-fit mt-2 md:mt-0"></span></div><p class="text-slate-700 font-bold mb-4 break-keep"></p><div class="grid grid-cols-1 md:grid-cols-2 gap-4"><div class="bg-white p-4 rounded-xl border border-slate-100"><span class="text-xs text-slate-400 block mb-1">전체 평점</span><span class="text-lg font-bold text-slate-800"></span><span class="text-xs text-slate-400 ml-1">(학점 이수)</span></div><div class="bg-white p-4 rounded-xl border border-slate-100"><span class="text-xs text-slate-400 block mb-1">전공 평점</span><span class="text-lg font-bold text-primary-600"></span><span class="text-xs text-slate-400 ml-1">(학점 이수)</span></div></div></div></div>`,
    'training': '',
    'activities': '',
